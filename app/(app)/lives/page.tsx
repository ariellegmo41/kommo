...
import {
  Video, Plus, Calendar, Clock, TrendingUp, ShoppingBag,
...
export default function LivesPage() {
  ...
  const totalRevenue   = lives.filter(l => l.revenue).reduce((s, l) => s + (l.revenue ?? 0), 0);
  const totalOrders    = lives.filter(l => l.orders).reduce((s, l) => s + (l.orders ?? 0), 0);
  const encerradas     = lives.filter(l => l.status === "encerrada");
  const avgConversion  = encerradas.length
    ? Math.round(encerradas.reduce((s, l) => s + (l.conversion ?? 70), 0) / encerradas.length)
    : 0;
  const liveNow = lives.find(l => l.status === "ao-vivo");
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              {encerradas.map((live) => (
                <div key={live.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-6 flex-shrink-0">
                    ...
                    <button className="text-gray-400 hover:text-gray-600 flex items-center gap-1 text-xs font-medium">
        ...
        <div>
          ...
          <div className="space-y-3">
            {lives.filter(l => l.status === "planejada").map((live) => {
            ...
            <button className="w-full py-4 flex items-center justify-center gap-2 text-[#6C3BFF] bg-[#6C3BFF]/5 hover:bg-[#6C3BFF]/10 rounded-xl border-2 border-dashed border-[#6C3BFF]/30 hover:border-[#6C3BFF]/50 transition-all text-sm font-medium">
